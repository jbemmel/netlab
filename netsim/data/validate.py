...
def validate_item(
  ...
  # JvB: For service extension attributes, validate service_type constraint if specified
  if 'service_type' in validation_attr:
    valid_service_type = validation_attr.pop('service_type',None)
    if not service_type:
      log.error(
        f'Unable to validate "service_type": {valid_service_type} for {data_name} attribute({parent_path}): No "type" in parent context',
        log.MissingValue,
        module)
      return False
    if ((type(valid_service_type) == str and valid_service_type!=service_type) 
        or (type(valid_service_type) is list and service_type not in valid_service_type)):
      log.error(
        f'Attribute {key} not valid for service.type=="{service_type}", only allowed for {valid_service_type} {parent_path}',
        log.IncorrectAttr,
        module)
      return False
...
def validate_attributes(
      ...
      service_type: typing.Optional[str] = None         # Service 'type'
        ) -> typing.Any: 
    ...
    if not modules is None and k in modules:            # For module attributes, perform recursive check
      ...
      if data[k] is True:
        if set(attr_list) & set(topology.defaults[k].attributes.can_be_true):
          continue
      service_type = data['type'] if 'type' in data and 'service' in attr_list else None
