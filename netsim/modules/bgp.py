  ...
  for l in node.get("interfaces",[]):
    ...
    for ngb_ifdata in l.get("neighbors",[]):
      ...
      if not af_list:                                                 # Neighbor has no usable address
        continue
      #
      # Get neighbor node data and its true or interface-local AS, TODO handle VRF case?
      #
...
#      print(f'EBGP node {node.name} neighbor {ngb_name} lla {ipv6_lla} v6num {ipv6_num} v4unnum {ipv4_unnum} rfc8950 {rfc8950}')
...
#      print(f'... unnumbered {unnumbered}')
      ...
      if not ebgp_data is None:
        ...
        if 'vrf' in l:        # VRF neighbor
          if not 'neighbors' in node.vrfs[l.vrf].bgp:
